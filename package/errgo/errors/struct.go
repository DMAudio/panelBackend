...
func (e *errorInfo) Error() string {
	...
	for {
		output.WriteString("\n")
		file, line := next.Location()
		if next.underlying != nil {
			if underlyingInfo, ok := next.underlying.(*errorInfo); ok {
				output.WriteString("" + underlyingInfo.message)
			} else {
				output.WriteString("" + next.underlying.Error())
			}
			if next.message != "" {
				output.WriteString("\n\t")
				output.WriteString("" + next.message)
			}
		} else if next.message != "" {
			output.WriteString("" + next.message)
		} else {
			output.WriteString("<no message>")
		}
		output.WriteString("\n\t")
		output.WriteString(fmt.Sprintf("%s:%d", file, line))
		if next.cause != nil {
			if nextInfo, ok := next.cause.(*errorInfo); ok {
				next = nextInfo
				continue
			} else {
				output.WriteString("\n")
				output.WriteString(next.cause.Error() + reflect.TypeOf(next.cause).String())
			}
		}
		break
	}
