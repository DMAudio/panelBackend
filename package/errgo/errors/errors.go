...
func Is(err error) func(error) bool {
	if errReal, ok := err.(*errorInfo); ok {
		if errReal.underlying != nil {
			err = errReal.underlying
		} else if errReal.cause != nil {
			err = errReal.cause
		}
	}
	return func(err1 error) bool {
		if errReal, ok := err1.(*errorInfo); ok {
			if errReal.underlying != nil {
				err1 = errReal.underlying
			} else if errReal.cause != nil {
				err1 = errReal.cause
			}
		}
